    ...
    for nx in [5, 6, 7, 8, 9, 10]:
        for ny in [5, 6, 7, 8, 9, 10]:
            for _ in range(3):
                ...
                [p_g1.append(np.random.rand()) for _ in range(nx)]
                [p_g2.append(np.random.rand()) for _ in range(nx)]
                [p_g3.append(np.random.rand()) for _ in range(nx)]

                alpha_g1, alpha_g2, alpha_g3 = [], [], []  # determines to what extent inactive constraint are satisfied
                [alpha_g1.append(np.random.rand()) for _ in range(nx)]
                [alpha_g2.append(np.random.rand()) for _ in range(nx)]
                [alpha_g3.append(np.random.rand()) for _ in range(nx)]
                mu_g1, mu_g2, mu_g3 = [], [], []  # random number in (0,1 ) with uniform probability to activ. comstr.
                [mu_g1.append(np.random.rand()) for _ in range(nx)]
                [mu_g2.append(np.random.rand()) for _ in range(nx)]
                [mu_g3.append(np.random.rand()) for _ in range(nx)]
                ...
                scale = np.array([1.2, .38])
                with open("scale.p", "wb") as f:
                    pickle.dump(scale, f)
