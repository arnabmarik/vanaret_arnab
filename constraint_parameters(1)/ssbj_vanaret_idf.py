...
from ssbj_vanaret_idf_mda import SsbjIdfMda


def idf_run(nx, ny):
    prob = Problem()  # initialize the optimization problem
    prob.model = SsbjIdfMda(nx_input=nx, ny_input=ny)  # create the MDA
    ...
    for tol in [1e-3, 1e-6]:
        ...
        if prob.driver.options['tol'] == 1e-6:
            ...
            df_idf = pickle.load(open("df_idf.p", "rb")).append(pd.DataFrame({'total iterations[IDF]': [iters],
            ...
            z = []
            [z.append(case.get_objectives(case)[obj_list[0]]) for case in case_ids]
            df_idf = pickle.load(open("df_idf_tol.p", "rb")).append(pd.DataFrame({'total iterations[IDF_tol]': [iters],
                                                                                  'total time[IDF_tol]': [total_time],
                                                                                  'final_objective[IDF_tol]': z[-1]}))
            df_idf.reset_index()
            pickle.dump(df_idf, open("df_idf_tol.p", "wb"))
        ...
        [z.append(case.get_objectives(case)[obj_list[0]]) for case in case_ids]
        print(z)
        if prob.driver.options['tol'] == 1e-3:
