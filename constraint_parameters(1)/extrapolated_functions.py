...
class Performance:
    ...
    def __init__(self):
        ...
        with open("component_dependency.p", "rb") as f:
            self.component_dependency = pickle.load(f)
        with open("obj_count.p", "rb") as f:
            obj_count = pickle.load(f)
            obj_count += 1
        with open("obj_count.p", "wb") as f:
    ...
    def range(self, nx, ny, x_des):
        ...
        return -1 * (np.mean(range14) + np.mean(range24) + np.mean(range34))
...
# # s3 = Propulsion()
# s4 = Performance()
# a = pickle.load(open("input_values.p", "rb"))
# nx = a[0]
# ny = a[1]
# print (nx, ny)
# # nx, ny = 3, 4
# # # # # c = np.ones(nx) * 1
# b = s4.range(nx, ny, .4 * np.ones(4 * nx + 5 * ny))
# print(b)
