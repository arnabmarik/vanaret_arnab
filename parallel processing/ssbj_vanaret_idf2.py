...
def idf_run2(nx, ny):
    ...
    for tol in [1e-3]:
        ...
        if prob.driver.options['tol'] == 1e-6:
            ...
            [z.append(case.get_objectives(case)[obj_list[0]]) for case in case_ids]
            df_idf = pickle.load(open("df_idf_tol.p", "rb")).append(pd.DataFrame({'10.total iterations[IDF_tol]':
                                                                                      [iters],
                                                                                  '11.total time[IDF_tol]':
                                                                                      [total_time],
                                                                                  '12.final_objective[IDF_tol]': z[-1],
                                                                                  '13.str_count[IDF_tol]': str_count,
                                                                                  '14.aer_count[IDF_tol]': aer_count,
                                                                                  '15.pro_count[IDF_tol]': pro_count
                                                                                  }))
