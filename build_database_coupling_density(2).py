...
for i in [.5]:
    ...
    for j in [3, 4, 5, 6, 7, 8]:
        for _ in range(4):
            nx = 20  # dimension of discipline inputs
            ...
            while a != 12 * ny + 3 * nx:  # : need to ensure a matrix is formed with no empty rows
                ...
                for h in range(12 * ny + 3 * nx):
                    if 1 in dependency_matrix[h]:
                        a += 1
            component_dependency = v2.build_component_dependency(nx, ny)  # remap extrapolations to original components
            pickle.dump(component_dependency, open("component_dependency.p", "wb"))  # pickle component dependency graph
           ...
           # plt.show()
            ...
            result = pd.concat([df_mdf, df_idf, df_idf_tol], axis=1)
            result.to_csv('results(averaging out runs)/result(nx = 20)/result_new_values' + '(' + str(i) + ')' + '.csv')
# : look from 21
